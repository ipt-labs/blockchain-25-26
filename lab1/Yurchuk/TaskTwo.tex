...
\textbf{Screenshot point \#1:} Capture the Ubuntu version output confirming successful installation.
...
\textbf{Screenshot point \#2:} Capture the \texttt{geth version} output.
...
\textbf{Screenshot point \#3:} Capture the contents of \texttt{genesis.json} showing the configuration parameters.
...
\textbf{Screenshot point \#4:} Capture the initialization output confirming successful genesis state creation.
...
перестворимо з чистого каталогу без попередньо існуючих даних ланцюжка і у dev mode.
...
\textbf{Screenshot point \#5:} Capture the Geth console output showing the dev mode initialization and the 
pre-funded account address. Capture the account address, balance, and block number in the Geth console.
...
\textbf{Screenshot point \#6:} Capture the account creation output showing the new address and keystore file path. 
Capture the account creation output (second terminal) and the \texttt{eth.accounts} listing showing both addresses.
...
\textbf{Screenshot point \#7:} Capture the transaction hash, block number increment, and recipient balance of 5~ETH.
...
реальній мережі нам довелося б чекати, поки валідатор включить її (+- 12 блоків, або 2-3 хв).

\textbf{Screenshot point \#8:} Capture \texttt{eth.getBlock(1)} output showing the block structure.
...
\textbf{Screenshot point \#9:} Capture the second terminal showing the initialization of the second node with the same genesis file.
...
\textbf{Screenshot point \#10:} Capture the output of \texttt{net.listening} and \texttt{net.peerCount} in the Geth console.
...
\textbf{Screenshot point \#11:} Capture the contents of \texttt{genesis-pow.json} showing the new configuration parameters, especially the high \texttt{terminalTotalDifficulty}.
...
\textbf{Screenshot point \#12:} Capture the initialization output from both nodes showing "Successfully wrote genesis state" with matching genesis hashes.
...
\textbf{Screenshot point \#13:} Capture the enode URL from Node~1 and the output of \texttt{admin.addPeer()} in Node~2 showing a successful connection.
...
\textbf{Screenshot point \#14:} Capture unsuccessful attempts to use mining commands in the Geth console, confirming their deprecation in post-Merge versions.
...
\textbf{Screenshot point \#15, 16:} Capture \texttt{admin.nodeInfo} showing the complete node and protocol configuration.
